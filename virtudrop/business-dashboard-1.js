...
// isYdHarvestIdentity/YD_HARVEST_IDENTIFIERS are still used below to gate the
// driver-tab switch for YD Harvest's owner fill-in coverage -- unrelated to
// pricing, so that stays as-is.
...
function normalizedTokens(value) {
  return normaliseZoneText(value).split(' ').filter(Boolean);
}
...
function scoreVariantMatch(text, value, baseScore) {
  const target = normaliseZoneText(value);
...
function scoreAreaMatch(input, area = {}) {
  ...
  for (const variant of zoneAreaVariants(area.area_name || area)) {
  ...
  for (const alias of area.zone_area_aliases || area.aliases || []) {
    ...
    for (const variant of zoneAreaVariants(aliasValue)) {
    ...
    if (alias.street_hint) {
      for (const variant of zoneAreaVariants(alias.street_hint)) {
    ...
    window.printDeliveryLabel = async function(orderId, options = {}) {
      ...
      try {
        ...
        try {
          ...
          ctx.fillText('PROFESSIONAL  •  STRUCTURED  •  RELIABLE', logoX + (drawWidth / 2), logoY + drawHeight + 46);
          ctx.letterSpacing = '0px';
          headerRuleY = Math.max(520, logoY + drawHeight + 92);
        } catch (logoError) {
          ctx.fillStyle = teal;
          ...
          ctx.font = '600 24px Inter, Arial, sans-serif';
          ctx.letterSpacing = '2px';
          ctx.fillText('PROFESSIONAL  •  STRUCTURED  •  RELIABLE', 600, 350);
          ctx.letterSpacing = '0px';
        }
        ...
        ctx.fillText('Love our service?', 600, 1625);
        ...
        ctx.textAlign = 'left';
        ctx.fillText('@VirtuForms', 835, 1750);
...
window.editCatalogItem = function(itemId) {
  ...
  const modal = el('#catalogEditModal');
  if (!item || !modal) return;
  if (el('#catalogEditItemId')) el('#catalogEditItemId').value = item.id;
  if (el('#catalogEditItemName')) el('#catalogEditItemName').value = item.item_name || '';
  if (el('#catalogEditItemCost')) el('#catalogEditItemCost').value = Number(item.item_cost || 0).toFixed(2);
  if (el('#catalogEditItemNotes')) el('#catalogEditItemNotes').value = item.additional_notes || '';
  if (el('#catalogEditItemActive')) el('#catalogEditItemActive').value = item.active ? 'true' : 'false';
  modal.classList.add('active');
};

window.closeCatalogEditModal = function() {
  el('#catalogEditModal')?.classList.remove('active');
};

async function saveCatalogEditItem(event) {
  event.preventDefault();
  if (!business?.id) return window.vdNotify('Catalog Not Ready', 'Business account is still loading.', 'warning');
  const itemId = el('#catalogEditItemId')?.value || '';
  if (!itemId) return window.closeCatalogEditModal();
  const itemName = el('#catalogEditItemName')?.value.trim() || '';
  const itemCost = Number(el('#catalogEditItemCost')?.value || 0);
  const additionalNotes = el('#catalogEditItemNotes')?.value.trim() || null;
  const active = el('#catalogEditItemActive')?.value !== 'false';
  ...
  const payload = { item_name: itemName, item_cost: itemCost, additional_notes: additionalNotes, active };
  ...
  window.vdNotify('Catalog Saved', itemName + ' was updated.', 'success');
}
...
function bindUi() {
  ...
  el('#catalogEditForm')?.addEventListener('submit', saveCatalogEditItem);
  el('#catalogEditModal')?.addEventListener('click', event => {
    if (event.target.id === 'catalogEditModal') window.closeCatalogEditModal();
  });
